...
const ManageInventory = () => {
  ...
  // State to toggle the full-screen Sticker Print View
    ...
    return (
      <div className="bg-white min-h-screen p-8 print:p-0 print:m-0">
        ...
        <div className="flex justify-between items-center mb-8 print:hidden border-b pb-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Print Barcode Stickers</h2>
            <p className="text-sm text-slate-500">Press Print to generate labels for all items.</p>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => setShowStickerPrint(false)}>
              Back to Dashboard
            </Button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 print:grid-cols-4 print:gap-4 print:bg-white">
          {items.map(item => 
            item.instances?.map((inst, idx) => (
              inst.controlNumber && (
                <div key={`${item.id}-${idx}`} className="flex flex-col items-center justify-center p-4 border rounded-lg bg-white break-inside-avoid shadow-sm print:shadow-none print:border-slate-300">
    ...
    <div className="bg-white p-6 m-5 rounded-lg border-2 w-full">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-3 print:hidden">
          <Button variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50" onClick={() => setShowStickerPrint(true)}>
          ...
          <Dialog open={isModalOpen} onOpenChange={(open) => { setIsModalOpen(open); if (!open) setFormData(getInitialForm()); }}>
            <DialogTrigger asChild>
              <Button className="bg-pink-600 hover:bg-pink-700 text-white"><Plus className="w-4 h-4 mr-2"/> Add Inventory</Button>
            </DialogTrigger>
              ...
              <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                ...
                <div className="pt-4 border-t mt-4">
                  <label className="text-xs font-bold text-pink-600 uppercase mb-3 block">
                    {isIndividualItems ? `Assign Control Numbers & Condition (${formData.instances.length} Pieces)` : "Item Tracking *"}
                  </label>
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              ...
              <TableHead className="min-w-[350px] w-[400px]">Instances (Control Number)</TableHead>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <TableRow key={item.id} className="items-start">
                  ...
                  <TableCell className="font-medium text-slate-800 align-top py-4">{item.name}</TableCell>
                  <TableCell className="align-top py-4">
                          ...
                          <AccordionContent>
                            <div className="max-h-[350px] overflow-y-auto w-full rounded-md border p-2 bg-slate-50/50">
                              <div className="space-y-4">
                                {item.instances.map((inst, idx) => (
                                  <div key={idx} className="flex flex-col gap-2 p-3 rounded-md border bg-white shadow-sm">
                                    <div className="flex justify-between items-center">
                                      <span className="font-semibold font-mono text-sm text-slate-800">
                                        {inst.controlNumber}
                                      </span>
                                      <span className={`ml-3 text-[11px] px-2 py-0.5 rounded border font-medium
                                    ...
                                    {inst.controlNumber ? (
                                      ...
                                      <span className="text-xs text-slate-400 italic">No control number</span>
                                    )}
                                  </div>
                                ))}
                              </div>
          ...
          <form onSubmit={handleEditSubmit} className="mt-4 space-y-4">
            ...
            <div className="pt-4 border-t mt-4">
              <label className="text-xs font-bold text-blue-600 uppercase mb-3 block">
                {isEditIndividualItems ? `Assign Control Numbers & Condition (${editFormData.instances.length} Pieces)` : "Item Tracking *"}
              </label>
