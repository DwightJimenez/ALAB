...
const ManageQuestions = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/quiz/admin/question/${questionToDelete}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 p-4 space-y-6">
      ...
      {/* HEADER SECTION */}
      <div className="flex justify-between items-center bg-white/10 p-6 rounded-xl border border-white/20 backdrop-blur-md shadow-sm">
        <div>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <Button onClick={openAddPanel} className="bg-navy hover:bg-cold text-white shadow-md">
            + Add New Question
          </Button>
          ...
          >
            ...
            <form onSubmit={handleSaveQuestion} className="space-y-4">
              <div className="space-y-2">
                <Label className="text-black font-semibold">Select Laboratory Skill</Label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                {[
                  ...
                  <div key={opt.key} className="space-y-2">
                    <Label className="text-black uppercase text-xs font-bold">{opt.label}</Label>
                    <Input
              ...
              <div className="space-y-2 pt-2">
                ...
                >
                  ...
                  <SelectContent className="bg-white border-gray-200 text-black">
                    {["A", "B", "C", "D"].map((v) => (
                      <SelectItem key={v} value={v} className="font-bold text-green-700">Option {v}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>

              <Button type="submit" className="w-full bg-navy hover:bg-cold mt-6 text-white h-12 text-lg">
      ...
      <div className="space-y-4">
        {questions.length === 0 ? (
          ...
          questions.map((q) => (
            <Card key={q.id} className="bg-white border-gray-200 shadow-sm hover:shadow transition-shadow">
              <CardContent className="p-5">
                <div className="flex justify-between items-start mb-4">
                  <div className="space-y-2">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 leading-tight">{q.text}</h3>
                  </div>
                  <div className="flex gap-2 shrink-0 ml-4">
                    <Button variant="outline" size="sm" onClick={() => openEditPanel(q)} className="border-gray-300">
                      Edit
                    </Button>
                    <Button variant="destructive" size="sm" onClick={() => setQuestionToDelete(q.id)}>
                      Delete
                    </Button>
                  </div>
                </div>
                  ...
                  {q.options.map((opt, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {opt}
                        {isCorrect && <span className="ml-2 text-green-600 font-bold float-right">✓ Correct</span>}
                      </div>
                    );
                  })}
                </div>
