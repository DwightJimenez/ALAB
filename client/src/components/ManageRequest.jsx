...
const ManageRequests = () => {
  ...
  const handleApprove = async () => {
    ...
    if (
      isIndividual &&
      assignedInstances.length !== selectedRequest.amountRequested
    ) {
      setActionError(
        `You must select exactly ${selectedRequest.amountRequested} control number(s).`,
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/requests/${selectedRequest.id}/approve`,
        {
          method: "PUT",
  ...
  const confirmReject = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/requests/${requestToReject}/reject`,
        {
          method: "PUT",
          credentials: "include",
        },
  ...
  const handleProcessReturn = async () => {
    ...
    if (
      isIndividual &&
      returnInstances.length !== returnRequest.amountRequested
    ) {
      setActionError(
        `You must evaluate exactly ${returnRequest.amountRequested} item(s) to process the return.`,
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/requests/${returnRequest.id}/return`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  return (
    <div className="bg-white p-6  rounded-lg border-2  w-full">
                  ...
                  pendingRequests.map((req) => (
                    <TableRow key={req.id}>
                      ...
                      <TableCell className="text-center font-bold text-pink-600">
                        {req.amountRequested} {req.inventory?.unit}
                      </TableCell>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          {selectedRequest && (
            <div className="space-y-4">
              ...
              {actionError && (
                <p className="text-red-500 text-sm font-bold">{actionError}</p>
              )}
              {selectedRequest.inventory?.category !== "CHEMICAL" && (
                <div>
                  <label className="text-sm font-bold text-slate-700 block mb-2">
                    Select {selectedRequest.amountRequested} Control Number(s):
                  </label>
                      ...
                      {selectedRequest.inventory?.instances?.map((inst) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            className="mr-3"
                            checked={assignedInstances.includes(inst.id)}
      ...
      <Dialog
        open={!!returnRequest}
        onOpenChange={(open) => !open && setReturnRequest(null)}
      >
        <DialogContent className="sm:max-w-[550px]">
              ...
              <div className="bg-blue-50 p-4 rounded border border-blue-100">
                <p className="text-sm text-slate-600">
                  ...
                  <span className="font-bold text-slate-900">
                    {returnRequest.student?.name}
                  </span>
                </p>
                <p className="text-sm text-slate-600">
                  Item:{" "}
                  <span className="font-bold text-slate-900">
                    {returnRequest.inventory?.name}
                  </span>
                </p>
                <p className="text-sm text-slate-600">
                  Quantity to Return:{" "}
                  <span className="font-bold text-blue-600">
                    {returnRequest.amountRequested}{" "}
                    {returnRequest.inventory?.unit}
                  </span>
                </p>
              </div>
                    ...
                    <div className="space-y-3">
                      {returnRequest.inventory?.instances?.length > 0 ? (
                        returnRequest.inventory.instances.map((inst) => {
                          const selectedData = returnInstances.find(
                            (r) => r.id === inst.id,
                          );
                          ...
                          return (
                            <div
                              key={inst.id}
                              className={`flex items-center justify-between p-3 rounded border transition-colors ${isSelected ? "bg-white border-blue-400 shadow-sm" : "bg-white border-slate-200"}`}
                            >
                              <div>
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleReturnToggle(inst, "Damaged")
                                  }
                                >
                                  Damaged
                                </Button>
                        ...
                        <p className="text-sm text-red-500 p-2">
                          No items found in "In Use" state.
                        </p>
                      )}
                    </div>
                  </ScrollArea>
                  <p className="text-xs font-bold text-slate-500 mt-2 text-right">
                    Processed: {returnInstances.length} /{" "}
                    {returnRequest.amountRequested}
                  </p>
                </div>
              ) : (
                <p className="text-sm text-slate-600 italic">
                  Chemicals are assumed consumed. Processing return will archive
                  this request but will <b>not</b> add the volume back to stock.
                </p>
              )}
            </div>
          )}
          ...
          <DialogFooter className="mt-6">
            <Button variant="ghost" onClick={() => setReturnRequest(null)}>
              Cancel
            </Button>
            <Button
              className="bg-blue-600 hover:bg-blue-700 text-white"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-600">
              Are you sure you want to reject this request? The student will be
