...
const DraggableChemical = ({ chemical, isMobile }) => {
  ...
  const handleClick = () => {
    if (isMobile) {
      window.dispatchEvent(new CustomEvent('chemical-selected', { detail: chemical.id }));
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center transition-opacity duration-300 group-hover:opacity-0">
        <div className={`absolute top-2 right-2 w-3 h-3 rounded-full shadow-sm ${chemical.color}`} />
...
const ReactionVisualizer = ({ effect, contents }) => {
  const liquidHeight = contents.length === 0 ? "h-0" : contents.length === 1 ? "h-1/3" : "h-2/3";
  ...
  if (contents.length === 1) {
    ...
    if (chem && chem.type !== 'solid' && chem.type !== 'gas') {
      liquidColor = chem.color.replace('bg-', 'bg-').replace('-100', '-300/60').replace('-200', '-400/60');
    }
  }
  ...
  if (effect === "neutralization") {
    ...
    particles = Array(10).fill().map((_, i) => (
      <Flame key={i} className="absolute text-orange-600 w-8 h-8 animate-ping" style={{ left: `${Math.random() * 80}%`, top: `${Math.random() * 80}%` }} />
    ));
  } else if (effect === "bubbling") {
    liquidColor = "bg-amber-200/70";
    particles = Array(15).fill().map((_, i) => (
      <Droplets key={i} className="absolute text-white w-5 h-5 animate-bounce" style={{ left: `${Math.random() * 80}%`, top: `${Math.random() * 80}%`, animationDuration: `${0.4 + Math.random()}s` }} />
    ));
  } else if (effect === "precipitate") {
    liquidColor = "bg-slate-300/70";
    particles = [<div key="solid" className="absolute bottom-0 w-full h-1/4 bg-slate-100/90 rounded-b-[2.5rem] border-t-2 border-slate-300"></div>];
  ...
  return (
    <div className="relative w-56 h-72 border-[6px] border-slate-300 rounded-b-[3rem] rounded-t-xl mx-auto flex items-end justify-center overflow-hidden bg-white/50 backdrop-blur-sm shadow-inner">
      <div className={`absolute bottom-0 w-full ${liquidHeight} ${liquidColor} ${animationClass} rounded-b-[2.5rem]`}>
         {particles}
      </div>
...
export default function ChemistryLabSandbox() {
  ...
  return (
    <div className="h-screen w-full bg-slate-50 text-slate-800 font-sans flex flex-col overflow-hidden selection:bg-blue-200">
      ...
      <header className="flex justify-center items-center px-6 py-4 bg-white border-b border-slate-200 z-10 shadow-sm">
      ...
      <main className="flex-1 relative flex flex-col items-center justify-center p-6 lg:p-12 overflow-y-auto">
        ...
        >
          <ReactionVisualizer effect={reactionResult?.visualEffect} contents={flaskContents} />
          ...
          <div className="h-16 mt-8 flex items-center justify-center font-mono text-4xl font-bold tracking-widest text-slate-700">
            {flaskContents.length === 0 ? (
              ...
              <span>
                {flaskContents[0]} {flaskContents.length > 1 && <span className="text-slate-400 mx-4">+</span>} {flaskContents[1]}
              </span>
            )}
          </div>

          <div className={`w-full mt-6 transition-all duration-300 ease-out transform ${reactionResult ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 pointer-events-none'}`}>
            {reactionResult && (
              <div className="bg-white rounded-3xl p-6 lg:p-8 border border-slate-200 shadow-xl flex flex-col items-center text-center">
                <div className="flex items-center gap-3 mb-2">
                  {reactionResult.visualEffect !== 'inert' && <Sparkles className="w-8 h-8 text-blue-500" />}
                  <h3 className="font-bold text-3xl text-slate-800">{reactionResult.productName}</h3>
                </div>
                
                <p className="text-slate-500 text-lg mb-6 font-medium bg-slate-100 px-4 py-1 rounded-full">{reactionResult.reactionType}</p>
      ...
      <footer className="bg-white/80 backdrop-blur-lg border-t border-slate-200 p-4 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)] flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl grid grid-rows-3 grid-flow-col gap-3 overflow-x-auto pb-4 px-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent">
          {filteredChemicals.length > 0 ? (
            ...
            <p className="text-slate-400 italic py-4 col-span-full">No chemicals found.</p>
          )}
        </div>
      </footer>
    </div>
