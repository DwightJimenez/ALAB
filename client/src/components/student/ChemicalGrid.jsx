      ...
      for (const chemicalName of standardChemicals) {
        try {
          ...
          if (safetyResponse.ok) {
            ...
            try {
              const hazardSection =
                safetyJson.Record.Section[0].Section[0].Section[0].Information;
              const hazardStatements = hazardSection.find(
                (info) => info.Name === "GHS Hazard Statements",
              );
              if (hazardStatements && hazardStatements.Value.StringWithMarkup) {
                safetyWarnings = hazardStatements.Value.StringWithMarkup.map(
          ...
          results.push({
            ...
            hazards:
              safetyWarnings.length > 0
                ? safetyWarnings
                : ["No specific GHS hazard warnings found."],
          });
        } catch (err) {
    ...
    <div className='max-w-7xl mx-auto p-6'>
      ...
      <div className='mb-8 border-b pb-4'>
        <h2 className='text-2xl font-bold text-slate-800'>
          Laboratory Chemical Database
        </h2>
        <p className='text-slate-500'>
          Properties and safety data sourced automatically via PubChem API.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className='flex flex-col items-center justify-center p-20 gap-4'>
          ...
          <div className='text-slate-500 font-medium'>
            Fetching data from National Institutes of Health...
          </div>
        </div>
      ) : (
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700'>
          {chemicalsData.map((chem, index) => (
            <div
              key={index}
              className='flex flex-col sm:flex-row bg-white border rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden'
            >
              {/* Left Side: 2D Structure Image */}
              <div>
                <div className='w-full sm:w-40 bg-slate-50 border-r flex flex-col items-center justify-center p-0 shrink-0 h-40 overflow-hidden'>
                  <img
                    src={`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${chem.cid}/PNG`}
                    alt={`2D structure of ${chem.name}`}
                    className='w-full h-full object-cover scale-150 drop-shadow-sm mix-blend-multiply'
                  />
                </div>
                  ...
                  <div className='max-h-24 overflow-y-auto pr-2 custom-scrollbar'>
                    <ul className='list-disc pl-4 space-y-1'>
                      {chem.hazards.map((hazard, hIndex) => (
                        <li
                          key={hIndex}
                          className='text-xs text-red-700 font-medium leading-snug'
                        >
                          {hazard}
                        </li>
                      ))}
                ...
                <SheetHeader className='mb-6 shrink-0 flex flex-row items-center justify-between'>
                  <div>
                    ...
                    <SheetDescription className='text-slate-500 text-base mt-2 flex items-center gap-2'>
                      ...
                      <span>
                        Molecular Weight: {selectedItem.molecularWeight} g/mol
                      </span>
                    </SheetDescription>
                  </div>
                </SheetHeader>
                  ...
                  <div className='w-full md:w-2/5 flex flex-col'>
                    <div className='bg-red-50 border border-red-100 rounded-xl p-5 flex flex-col h-full max-h-[50vh] md:max-h-[70vh]'>
                      ...
                      <div className='overflow-y-auto pr-2 custom-scrollbar flex-1'>
                        {selectedItem.hazards.length === 1 &&
                        selectedItem.hazards[0].includes("No specific") ? (
                          <p className='text-slate-600 italic text-sm'>
                            {selectedItem.hazards[0]}
                          </p>
                        ) : (
                          <ul className='list-disc pl-5 space-y-2'>
                            {selectedItem.hazards.map((hazard, hIndex) => (
                              <li
                                key={hIndex}
                                className='text-sm text-red-700 font-medium leading-relaxed'
                              >
                                {hazard}
                              </li>
                            ))}
