...
import GateScanner from "./GateScanner"; // <-- Adjust this path if necessary
...
const PassedList = () => {
  ...
  // NEW: State to toggle between the list and the scanner
  const [activeView, setActiveView] = useState("list"); // 'list' | 'scanner'

  const API_URL = import.meta.env.VITE_API_URL;

  useEffect(() => {
    // We only need to fetch this if we are in the list view,
    // but fetching on mount keeps it fast.
    ...
    fetchStudentStatus();
  }, [activeView]); // Re-fetch when switching back to the list
          ...
          {isLoading ? (
            ...
            <Accordion type='multiple' className='space-y-4'>
              {processedStudents.map((student) => {
                ...
                const pendingSkills = student.skills.filter(
                  (s) => !s.isMastered,
                );
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mt-4'>
                        <div>
                          <h4 className='font-semibold text-green-800 flex items-center gap-2 mb-3 border-b border-green-100 pb-2'>
                            <CheckCircle2 size={18} /> Passed Skills (
                            {passedSkills.length})
                          </h4>
                          {passedSkills.length === 0 ? (
                            <p className='text-sm text-slate-400 italic'>
                              No skills passed yet.
                            </p>
                          ) : (
                            <ul className='space-y-2'>
                              {passedSkills.map((skill) => (
                                <li
                                  key={skill.id}
                                  className='text-sm text-slate-700 flex items-center gap-2 bg-green-50/50 p-2 rounded-md border border-green-100/50'
                                >
                        ...
                        <div>
                          <h4 className='font-semibold text-amber-800 flex items-center gap-2 mb-3 border-b border-amber-100 pb-2'>
                            <XCircle size={18} /> Pending Skills (
                          ...
                          {pendingSkills.length === 0 ? (
                            ...
                            <ul className='space-y-2'>
                              {pendingSkills.map((skill) => (
                                <li
                                  key={skill.id}
                                  className='text-sm text-slate-600 flex items-center gap-2 bg-slate-50 p-2 rounded-md border border-slate-100'
                                >
