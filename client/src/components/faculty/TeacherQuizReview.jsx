              ...
              <Card key={sIdx} className='border-indigo-100 shadow-sm bg-indigo-50/30'>
                ...
                <CardContent>
                  <div className='grid grid-cols-4 gap-2'>
                    {["p_init", "p_transit", "p_slip", "p_guess"].map((param) => (
                      <div key={param} className='space-y-1'>
                        ...
                        <Input
                          ...
                          max='1'
                          value={skill[param] || 0}
                          onChange={(e) => handleUpdateSkill(sIdx, param, e.target.value)}
      ...
      {questions.length > 0 && (
        <div className='space-y-8 border-t pt-8'>
          ...
          <div className="space-y-10">
            {Object.entries(groupedQuestions).map(([skillName, skillQuestions]) => (
              <div key={skillName} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {skillQuestions.map((q) => (
                    <Card key={q.originalIndex} className='shadow-sm border-slate-200 flex flex-col h-full'>
                      <CardHeader className='pb-3'>
                        <Input
                          className='text-sm font-medium'
                          value={q.questionText}
                          onChange={(e) =>
                            handleUpdateQuestionText(q.originalIndex, e.target.value)
                          }
                        />
                      </CardHeader>
